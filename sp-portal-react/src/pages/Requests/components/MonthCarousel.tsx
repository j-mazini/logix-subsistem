...
export const MonthCarousel = memo<MonthCarouselProps>(({ months, onMonthClick }) => {
  ...
  return (
    <section className={requestsStyles.monthCarouselSection}>
      <div
        ref={scrollContainerRef}
        className={requestsStyles.monthCarouselScroll}
      >
        <div
          className={requestsStyles.monthCarouselItems}
            ...
            return (
              <motion.button
                ...
                className={`carousel-month-item${active ? "-active" : ""} ${
                  requestsStyles.monthCarouselButtonBase
                } ${
                  active
                    ? requestsStyles.monthCarouselButtonActive
                    : requestsStyles.monthCarouselButtonInactive
                }`}
                onClick={() => {
                  if (!active) {
              ...
              >
                <span className={requestsStyles.monthCarouselButtonLabel}>
                  <span>{item.label.split('/')[0]}</span>
                  <span className={requestsStyles.monthCarouselButtonYear}>
                    {item.year}
                  </span>
                </span>
              </motion.button>
