// Ported from the Next.js source's utils/pdfExport.ts + pdfGenerator.ts,
// which both built a styled "Vehicles Report" with jsPDF (installed as an
// npm dependency there, loaded via a shared `@/lib/lazy-imports` helper
// backed by a real backend API).
//
// This is a mock-data SPA with no npm jsPDF dependency, so this port
// follows the CDN-loading convention already established in
// src/pages/Vetting/checklist/modules/client-outputs/exporters.ts: load
// jsPDF from a CDN <script> tag on demand, and fall back to an
// open-a-printable-window flow (window.print()) if the CDN load fails
// (e.g. offline demo/dev environment).
import type { RawVehicle } from '../../../data/vehiclesData';
import { formatDateDisplay } from '../../../data/vehiclesData';
...
/** Styled "Vehicles Report" PDF, ported from the Next.js source's pdfGenerator.ts. */
export async function generateVehiclesPDF({ vehicles, serviceProvider, statusFilter, fuelFilter, search }: VehiclesPdfParams): Promise<void> {
  try {
    ...
    doc.text('Vehicle Fleet Management', margin + logoSize + 8, margin + 14);
    ...
    doc.setFontSize(11);
    doc.setFont(undefined, 'bold');
    doc.text('VEHICLES REPORT', titleBoxX + titleBoxWidth / 2, margin + 10, { align: 'center' });
    ...
    doc.text(`Total Vehicles: ${vehicles.length}`, margin + 5, currentY + 22);
    ...
    if (fuelFilter !== 'all') {
      doc.text(`Fuel Type: ${fuelFilter}`, rightBoxX + 5, filterY);
      filterY += 6;
      hasFilters = true;
    }
    if (search) {
    ...
    const colWidths = [32, 32, 32, 38, 28, 28];
    ...
    vehicles.forEach((vehicle) => {
      ...
      const rowData = [
        ...
        vehicle.brand || '-',
        vehicle.model || '-',
        formatDateDisplay(vehicle.registrationDate),
        vehicle.fuelType || '-',
    ...
    console.error('Error generating vehicles PDF, falling back to print dialog:', error);
    window.print();
  }
}
