/** Date helpers — ported from the reference app's requests/utils.ts. */
 ...
 * Only for tomorrow is the 12:00 cutoff rule applied.
 */
export function canRequestDayOffForDate(requestedDateISO: string): { allowed: boolean; message?: string } {
  const now = new Date();
  ...
  if (requestedDateISO === tomorrowISO) {
    ...
    if (hour > 12 || (hour === 12 && minute > 0)) {
      return {
        allowed: false,
        message: 'Day off for tomorrow can only be requested until 12:00 today. It is too late to submit this request.',
      };
    }
  }
