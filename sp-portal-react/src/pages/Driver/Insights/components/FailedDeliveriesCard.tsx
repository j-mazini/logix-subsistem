...
import type { UnpaidStops } from '../types';
...
export function FailedDeliveriesCard({ unpaidStops }: FailedDeliveriesCardProps) {
  const deliveryTypes = [
  ...
  return (
    <div className={dailyPerformanceInsightStyles.cardBase}>
      <div className={dailyPerformanceInsightStyles.failedDeliveriesHeader}>
        ...
        <h3 className={dailyPerformanceInsightStyles.failedDeliveriesTitle}>ATTEMPTED FAILED DELIVERIES</h3>
      </div>
      <div className={dailyPerformanceInsightStyles.failedDeliveriesGrid}>
        {deliveryTypes.map((type, index) => (
          ...
          >
            ...
            <span className={dailyPerformanceInsightStyles.failedDeliveriesLabel}>{type.label}</span>
            <span className={dailyPerformanceInsightStyles.failedDeliveriesValue}>{type.value}</span>
          </motion.div>
        ))}
      </div>
    </div>
