...
import { calculateSPR, calculateSPORH, calculateAFD } from '../performanceUtils';
...
export function OperationCard({ operation, index, totalOperations }: OperationCardProps) {
  ...
  return (
    ...
    >
      ...
      {displayData.showMetrics && (
        <>
          <div className={dailyPerformanceInsightStyles.cardBase}>
            <MetricsCard spr={metrics.spr} sporh={metrics.sporh} tw={metrics.tw} afd={metrics.afd} />
          </div>
          <div className={dailyPerformanceInsightStyles.cardBase}>
      ...
      {index < totalOperations - 1 && <hr className={dailyPerformanceInsightStyles.operationCardDivider} />}
    </motion.div>
  );
}
