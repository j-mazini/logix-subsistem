...
export function MetricsCard({ spr, sporh, tw, afd }: MetricsCardProps) {
  ...
  const metrics = [
    ...
    {
      label: 'TW',
      value: formatPercentage(tw / 100),
      type: 'tw',
      icon: 'bi-clock',
      colorClass: getValueColorClass('tw', twColorClass),
    },
      ...
      {metrics.map((metric, index) => (
        ...
        >
          <div className={dailyPerformanceInsightStyles.metricsCardHeader}>
            ...
            <span className={dailyPerformanceInsightStyles.metricsCardLabel}>{metric.label}</span>
          </div>
