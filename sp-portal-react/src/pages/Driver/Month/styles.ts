// Ported from current-month/styles.ts. The reference's blue brand accent was
// swapped to emerald to match this Driver Portal's established accent (see
// PageLayout.tsx / layout.tsx MobileNavBar, both emerald-based).
export const currentMonthStyles = {
  card: 'bg-white rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all duration-300 mb-4 border border-slate-100',

  sectionTitle: 'text-lg font-bold text-slate-800 mb-3 uppercase text-sm flex items-center gap-2',
  sectionTitleIcon: 'bi bi-calendar-check text-emerald-600',
  ...
  rowBase:
    'flex justify-between items-center px-3 py-3 cursor-pointer transition-all duration-300 rounded-xl hover:bg-emerald-50 hover:shadow-sm',
  rowEven: 'bg-slate-50',
  ...
  dayDetailsRoot: 'bg-slate-100 text-slate-800 p-3 min-h-screen pb-[clamp(3.5rem,8vh,4.375rem)]',
  dayDetailsBackButton:
    'mb-4 inline-flex items-center gap-1 text-sm font-medium text-emerald-600 hover:text-slate-800 transition-colors',
  dayDetailsBackIcon: 'bi bi-arrow-left text-xs',
  dayDetailsScroll: 'max-h-[clamp(22rem,70vh,44rem)] overflow-y-auto pb-5 space-y-4',
  dayDetailsCard: 'bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden',

  routeHeader: 'bg-emerald-600 px-4 py-3 flex justify-between items-center rounded-t-2xl',
  routeHeaderTitle: 'text-lg font-semibold text-white',
  routeHeaderBadge: 'bg-emerald-900/40 text-white px-3 py-1.5 rounded-full text-xs font-semibold',

  metricsGrid: 'grid grid-cols-3 gap-3 p-4',
  metricsCard: 'bg-slate-50 border border-slate-200 rounded-lg p-3 text-center',
  metricsLabel: 'text-[0.7rem] font-semibold text-slate-500 mb-1 uppercase tracking-wide',
  metricsValue: 'text-xl font-bold text-slate-800',

  workHoursCard: 'mx-4 mb-4 bg-slate-50 border border-slate-200 rounded-lg p-4',
  workHoursLabel:
  ...
  workHoursTotalValue: 'text-2xl font-bold text-emerald-600',

  stopsSectionTitle: 'text-sm font-bold text-slate-800 mb-3 uppercase tracking-wide',
  stopsGrid: 'grid grid-cols-4 gap-2',
  stopsCard: 'bg-slate-50 border border-slate-200 rounded-lg py-2.5 px-2 text-center',
  stopsLabel: 'text-[0.7rem] font-semibold text-slate-500 mb-1 uppercase tracking-wide',
  stopsValue: 'text-base font-bold text-slate-800',

  paymentSectionTitle: 'text-sm font-bold text-slate-800 mb-3 uppercase tracking-wide',
  paymentRow: 'flex justify-between items-center py-3 border-b border-slate-200 last:border-b-0',
  ...
  noteCard: 'px-4 pb-4',
  noteInner: 'bg-amber-50 rounded-lg p-3 border-l-4 border-amber-200',
  noteTitle: 'text-sm font-bold text-amber-800 mb-2 uppercase tracking-wide',
  ...
  summaryCardLabel: 'text-sm font-medium text-slate-600 mb-2 relative z-10 flex items-center justify-center gap-2',
  ...
  loadingSpinner: 'inline-block animate-spin rounded-full h-8 w-8 border-4 border-emerald-500 border-t-transparent',
  ...
  noDataSection: 'bg-slate-50 rounded-xl p-4 shadow-[0_2px_8px_rgba(0,0,0,0.08)] mb-4',
