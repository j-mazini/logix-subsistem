...
interface VendorsOffPanelProps {
    isCollapsed: boolean;
    onToggleCollapse: () => void;
    vendorsOffCount: number;
    vendorsOffDisplay: Array<{
...
/**
 * Dedicated row component for the Vendors Off panel.
 * Column order: Name | Deposit | Default Route | Vehicle | Notes | Actions
 */
const VendorsOffRow = memo(({
    item,
    depositName,
    rawStatus,
    reason,
    ...
    const statusLabel = rawStatus || 'OFF';
        ...
        >
            ...
            <td className={dailyGamePlanStyles.tableCell}>
                <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[10px] uppercase tracking-wide text-red-700/80 font-semibold">{statusLabel}</span>
                </div>
            </td>
            ...
            <td className={dailyGamePlanStyles.tableCell}>
                {registrationPlate && registrationPlate !== '–' ? (
                    <div className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${getVehicleBadgeVariant?.(registrationPlate) === 'blue' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                        {registrationPlate}
                    </div>
                ) : (
                    <span className="text-gray-400 text-sm">-</span>
            ...
            <td className={dailyGamePlanStyles.tableCellLeft}>
                <div className={`${dailyGamePlanStyles.notesFieldWrapper} flex items-center gap-1`}>
                    <button
                        ...
                        title={reason ? `${noteValue ? 'View/Edit notes' : 'Add notes'} • ${reason}` : (noteValue ? 'View/Edit notes' : 'Add notes')}
                    >
...
VendorsOffRow.displayName = 'VendorsOffRow';

const VendorsOffPanel = memo(({
    isCollapsed,
    onToggleCollapse,
    vendorsOffCount,
    vendorsOffDisplay,
    ...
    getVehicleBadgeVariant,
    canEdit,
}: VendorsOffPanelProps) => {
    if (vendorsOffCount === 0) return null;

    return (
        <motion.div className="mb-8" layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="bg-white rounded-xl shadow-lg border border-red-100 overflow-hidden">
                <button onClick={onToggleCollapse} className="w-full px-6 py-4 flex items-center justify-between bg-gradient-to-r from-red-50 to-white hover:from-red-100/50 transition-colors">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-red-100 rounded-lg text-red-600">
                            <UserX className="h-5 w-5" />
                        </div>
                        <div>
                            <h3 className="text-sm font-bold text-red-900 uppercase tracking-wider">Vendors Off</h3>
                            <p className="text-xs text-red-700 font-medium">{vendorsOffCount} vendor{vendorsOffCount !== 1 ? 's' : ''} not working today</p>
                        </div>
                    </div>
                    {isCollapsed ? <ChevronDown className="h-5 w-5 text-red-400" /> : <ChevronUp className="h-5 w-5 text-red-400" />}
                </button>
                                ...
                                <table className="w-full min-w-[640px] table-auto">
                                    <thead>
                                        <tr className="bg-red-50/30 border-b border-red-100 text-[10px] font-bold text-red-800 uppercase tracking-widest">
                                            ...
                                            <th className="px-6 py-2.5 text-left w-[16%]">Deposit</th>
                                            <th className="px-6 py-2.5 text-left w-[18%]">Default Route</th>
                                            <th className="px-6 py-2.5 text-left w-[16%]">Vehicle</th>
                                            <th className="px-6 py-2.5 text-left w-[20%]">Notes</th>
                                            {canEdit && <th className="px-6 py-2.5 text-center w-[12%]">Actions</th>}
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-red-50">
                                        {vendorsOffDisplay.map(({ item, depositName, rawStatus, reason, routeDefaultName, registrationPlate, isVanHome }, idx) => (
                                            <VendorsOffRow
                                                key={`off-${item.dailyGamePlanOperationId ?? idx}`}
                                                item={item}
...
VendorsOffPanel.displayName = 'VendorsOffPanel';

export default VendorsOffPanel;
