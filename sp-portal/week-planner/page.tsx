...
function getDayNameShort(date: Date) {
...
function getWeekDatesFiltered(weekStart: Date, includeWeekends: boolean) {
  const all = getWeekDates(weekStart);
  return includeWeekends ? all : all.filter((_, i) => i < 5);
}
function formatWeekRangeLabel(weekStart: Date, includeWeekends: boolean) {
...
interface Route { id: number; depotId: number; name: string }
function buildRoutesForDepot(depot: (typeof DEPOTS)[number]): Route[] {
...
const VENDOR_FIRST_NAMES = ['James', 'Oliver', 'Harry', 'George', 'Jack', 'Charlie', 'Thomas', 'Jacob', 'Alfie', 'Freddie', 'Amelia', 'Olivia', 'Isla', 'Ava', 'Emily', 'Sophia', 'Grace', 'Mia', 'Poppy', 'Ella', 'Ryan', 'Liam', 'Noah', 'Ethan', 'Mason', 'Priya', 'Amir', 'Kofi', 'Lena', 'Marta'];
const VENDOR_LAST_NAMES = ['Smith', 'Johnson', 'Williams', 'Brown', 'Taylor', 'Davies', 'Evans', 'Wilson', 'Thomas', 'Roberts', 'Walker', 'Wright', 'Green', 'Hall', 'Wood', 'Patel', 'Khan', 'Osei', 'Kowalski', 'Novak'];
const VEHICLE_TYPES = ['3.5T Luton', '7.5T Box Truck', 'Transit Van', 'Sprinter Van', '18T Rigid', 'Flatbed Truck'];

interface PoolVendor { id: number; name: string; vehicle: string; plate: string }
...
const MOCK_NOTES = ['Vehicle arrived late.', 'Extra stop confirmed by depot.', 'Route completed early.', 'Traffic delay reported.', 'Customer redelivery required.', 'Covering for absent driver.'];

interface WPRecord {
  weekPlannerId: number; depotId: number; routeId: number | null; isSupervisor: boolean;
  date: string; name: string; vendorId: number; route: string; routeSort: 'yes' | 'no';
  notes: string; vehicle: string; registrationPlate: string; status: string; isDayOff: string; isSort: string;
}
  ...
  DEPOTS.forEach((depot) => {
    ...
    routes.forEach((route) => {
      weekDates.forEach((date) => {
        ...
        records.push({
          weekPlannerId: idCounter++, depotId: depot.id, routeId: route.id, isSupervisor: false, date: dateISO,
          name: vendor.name, vendorId: vendor.id, route: route.name, routeSort,
          notes: hasNotes ? MOCK_NOTES[Math.floor(rng() * MOCK_NOTES.length)] : '',
          vehicle: vendor.vehicle, registrationPlate: vendor.plate, status: 'Working',
          isDayOff: 'false', isSort: routeSort === 'yes' ? 'true' : 'false',
        });
      });
    });
    ...
    weekDates.forEach((date) => {
      ...
      if (!vendor) return;
      records.push({
        weekPlannerId: -(idCounter++), depotId: depot.id, routeId: null, isSupervisor: true, date: dateISO,
        name: vendor.name, vendorId: vendor.id, route: 'Team Leader', routeSort: 'no', notes: '',
        vehicle: vendor.vehicle, registrationPlate: vendor.plate, status: 'Working', isDayOff: 'false', isSort: 'false',
      });
    });
  });
...
export default function WeekPlannerPage() {
  useBodyClass('wp-page has-beam-sidebar');
  ...
  const [supervisorPopover, setSupervisorPopover] = useState<{ depotId: number; dateISO: string; left: number; top: number; vendorId: number } | null>(null);
  ...
  function getAssignedVendorIds() {
    const ids = new Set<number>();
    records.forEach((r) => ids.add(r.vendorId));
    return ids;
  }
  const availableVendors = useMemo(() => {
    const assigned = getAssignedVendorIds();
    return VENDOR_POOL.filter((v) => !assigned.has(v.id));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentWeekStart, records.length]);
  ...
  function handleDropOnCell(depotId: number, routeId: number | null, dateISO: string) {
    ...
    if (payload.type === 'vendor') {
      ...
      records.push({
        weekPlannerId: idCounter++, depotId, routeId, isSupervisor: false, date: dateISO,
        name: vendor.name, vendorId: vendor.id, route: route ? route.name : '', routeSort: 'no', notes: '',
        vehicle: vendor.vehicle, registrationPlate: vendor.plate, status: 'Working', isDayOff: 'false', isSort: 'false',
      });
    ...
    if (payload.type === 'assignment') {
      ...
      if (!rec) return;
      if (rec.depotId === depotId && rec.routeId === routeId && rec.date === dateISO) return;
  ...
  function saveSupervisor(vendorId: number) {
    ...
    if (existingIdx >= 0) {
      ...
      records.push({
        weekPlannerId: -(idCounter++), depotId, routeId: null, isSupervisor: true, date: dateISO,
        name: vendor.name, vendorId: vendor.id, route: 'Team Leader', routeSort: 'no', notes: '',
        vehicle: vendor.vehicle, registrationPlate: vendor.plate, status: 'Working', isDayOff: 'false', isSort: 'false',
      });
    }
  ...
  const filteredDrawerVendors = availableVendors.filter((v) => !vendorsDrawerSearch.trim() || v.name.toLowerCase().includes(vendorsDrawerSearch.trim().toLowerCase()));
  ...
  return (
    <div className="sidebar-wrapper" id="sidebarWrapper">
      ...
      <div className="sidebar-inset" id="sidebarInset">
        <main className="wp-container container-fluid px-3 px-lg-4 py-4">
          <header className="page-header-section">
            <div className="page-header-welcome-text">
              <h1 className="page-header-title">Week Planner</h1>
              <p className="page-header-date">
                <i className="bi bi-calendar-week" />
                <span>Week of {currentWeekStart.toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })} — {DEPOTS.length} depots</span>
              </p>
            </div>
            <div className="wp-header-metric">
              <p className="wp-header-metric-label">Available Vendors</p>
              <p className="wp-header-metric-value">{availableVendors.length}</p>
            </div>
          </header>

          <div className="filters-bar wp-controls-bar">
            <div className="week-nav">
              <span className="week-nav-label"><i className="bi bi-calendar3" /> Week</span>
              <div className="week-nav-controls">
                <button type="button" className="date-nav-btn" aria-label="Previous week" onClick={() => shiftWeek(-7)}><i className="bi bi-chevron-left" /></button>
                <button type="button" className="date-nav-btn week-nav-today" onClick={() => setCurrentWeekStart(getWeekStart(new Date()))}>This week</button>
                <button type="button" className="date-nav-btn" aria-label="Next week" onClick={() => shiftWeek(7)}><i className="bi bi-chevron-right" /></button>
                <span className="week-range-label">{formatWeekRangeLabel(currentWeekStart, showWeekends)}</span>
              </div>
            </div>
            ...
            <div className="wp-toggle-wrap">
              ...
              <span className="filters-label wp-toggle-label">{showWeekends ? 'Show weekends' : 'Weekdays only'}</span>
            </div>

            <div className="dfi-actions">
              <button type="button" className="styled-button styled-button--outline" onClick={() => setVendorsDrawerOpen(true)}>
                <i className="bi bi-people" /> Available Vendors <span className="wp-badge-count">{availableVendors.length}</span>
              </button>
            </div>
          </div>

          <div className="wp-legend">
            <span className="wp-legend-item"><span className="wp-legend-dot wp-legend-dot--sort" /> Route Sort</span>
            <span className="wp-legend-item"><span className="wp-legend-dot wp-legend-dot--supervisor" /> Team Leader</span>
            <span className="wp-legend-item"><span className="wp-plate wp-legend-plate">AB12 CDE</span> Vehicle registration</span>
          </div>

          <div className="wp-depot-list">
            {DEPOTS.map((depot) => {
              const routes = ROUTES_BY_DEPOT.get(depot.id)!;
              ...
              return (
                <section className={`wp-depot-section${collapsed ? ' collapsed' : ''}`} key={depot.id}>
                  <div className="wp-depot-header" onClick={() => {
                    ...
                    <div className="wp-depot-header-left">
                      <button type="button" className="wp-depot-collapse-btn" aria-label="Collapse depot"><i className="bi bi-chevron-down" /></button>
                      <div>
                        <h2 className="wp-depot-title">{depot.name}</h2>
                        <p className="wp-depot-subtitle">{routes.length} route{routes.length === 1 ? '' : 's'} scheduled</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="wp-depot-body">
                    <div className="wp-grid-scroll">
                      <div className="wp-grid" style={{ gridTemplateColumns: gridTemplate }}>
                          ...
                          return (
                            <div
                              ...
                              onClick={(e) => {
                                ...
                                setSupervisorPopover({ depotId: depot.id, dateISO, left, top, vendorId: supAssignment?.vendorId ?? 0 });
                              }}
                            >
                              {supAssignment ? (
                                <div className="wp-supervisor-chip" title={`${supAssignment.name} — click to edit`}>{supAssignment.name}</div>
                              ) : (
                        ...
                        <div className="wp-cell-head wp-head-corner" onClick={(e) => e.stopPropagation()}><span className="wp-day-name">Route</span></div>
                        {weekDates.map((date) => (
                          <div key={`head-${formatDateISO(date)}`} className={`wp-cell-head${isWeekend(date) ? ' is-weekend' : ''}`} onClick={(e) => e.stopPropagation()}>
                        ...
                        {routes.map((route) => (
                          <React.Fragment key={route.id}>
                            <div className="wp-cell-label" title={route.name} onClick={(e) => e.stopPropagation()}>{route.name}</div>
                            {weekDates.map((date) => {
                              const dateISO = formatDateISO(date);
                              ...
                              return (
                                <div
                                  ...
                                  onDragOver={(e) => { e.preventDefault(); e.dataTransfer.dropEffect = dragPayload.current?.type === 'vendor' ? 'copy' : 'move'; }}
                                  onDrop={(e) => { e.preventDefault(); handleDropOnCell(depot.id, route.id, dateISO); }}
                                >
                                  {cellRecords.length === 0 ? (
                                    ...
                                    cellRecords.map((rec) => (
                                      <div
                                        ...
                                        className={`wp-card${rec.routeSort === 'no' ? ' wp-card--sort-no' : ''}${rec.status === 'Day Off' || rec.status === 'OFF' ? ' wp-card--day-off' : ''}`}
                                        draggable
                                        title={`${rec.name} — click to edit`}
                                        onDragStart={(e) => { dragPayload.current = { type: 'assignment', id: rec.weekPlannerId }; e.dataTransfer.effectAllowed = 'move'; e.stopPropagation(); }}
                                      ...
                                      >
                                        ...
                                        <div className="wp-card-bottom">
                                          ...
                                          {rec.notes && <span className="wp-card-notes-flag" title={rec.notes}>!</span>}
                                        </div>
                                      </div>
                                    ))
                                  )}
      ...
      {vendorsDrawerOpen && <div className="wp-drawer-overlay" onClick={() => setVendorsDrawerOpen(false)} />}
      <aside className={`wp-vendors-drawer${vendorsDrawerOpen ? ' open' : ''}`} aria-hidden={!vendorsDrawerOpen}>
        <div className="wp-drawer-header">
          <h2><i className="bi bi-people" /> Available Vendors</h2>
          <button type="button" className="dom-modal-close" aria-label="Close" onClick={() => setVendorsDrawerOpen(false)}><i className="bi bi-x-lg" /></button>
        </div>
        <div className="wp-drawer-search">
          <div className="search-input-wrap">
            <i className="bi bi-search" />
            <input type="text" className="form-control" placeholder="Search vendor…" autoComplete="off" value={vendorsDrawerSearch} onChange={(e) => setVendorsDrawerSearch(e.target.value)} />
          </div>
        </div>
        <p className="wp-drawer-hint">Drag a vendor onto an empty day cell to assign them for that route and day.</p>
        <div className="wp-drawer-list">
          {filteredDrawerVendors.length === 0 ? (
            <p className="wp-drawer-empty"><i className="bi bi-check2-circle" /><br />No unassigned vendors match.</p>
          ) : (
            filteredDrawerVendors.map((v) => (
              <div
                ...
                onDragStart={(e) => { dragPayload.current = { type: 'vendor', vendorId: v.id }; e.dataTransfer.effectAllowed = 'copy'; }}
              >
                <div>
      ...
      {/* Supervisor Popover */}
      {supervisorPopover && (
        <div className="wp-popover" style={{ left: supervisorPopover.left, top: supervisorPopover.top }}>
          <p className="wp-popover-title">Team Leader — {DEPOTS.find((d) => d.id === supervisorPopover.depotId)?.name} ({supervisorPopover.dateISO})</p>
          <select className="form-select" value={supervisorPopover.vendorId || ''} onChange={(e) => setSupervisorPopover({ ...supervisorPopover, vendorId: Number(e.target.value) })}>
            <option value="">-- Unassigned --</option>
            {VENDOR_POOL.map((v) => <option value={v.id} key={v.id}>{v.name}</option>)}
          </select>
          <div className="wp-popover-actions">
            <button type="button" className="styled-button styled-button--outline" onClick={() => saveSupervisor(0)}>Clear</button>
            <button type="button" className="styled-button styled-button--primary" onClick={() => saveSupervisor(supervisorPopover.vendorId)}>Save</button>
          </div>
        </div>
      )}
...
function AssignmentModal({
  ...
  return (
    <div className="wp-modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="wp-modal">
        <div className="wp-modal-header">
          <h2 className="wp-modal-title"><i className="bi bi-pencil-square me-2" />Edit Assignment</h2>
          <button type="button" className="dom-modal-close" aria-label="Close" onClick={onClose}><i className="bi bi-x-lg" /></button>
        </div>
        <form className="wp-modal-body" onSubmit={(e) => { e.preventDefault(); onSave(vendorId, vehicle, plate, routeSort, status, notes); }}>
          ...
          <div className="dom-form-grid">
            <div className="dom-form-field span-2">
              ...
              <select className="form-select" value={vendorId} onChange={(e) => setVendorId(Number(e.target.value))}>
                {VENDOR_POOL.map((v) => <option value={v.id} key={v.id}>{v.name}</option>)}
              </select>
            </div>
            <div className="dom-form-field">
              <label className="dom-form-label">Vehicle</label>
              <input type="text" placeholder="e.g. 7.5T Box Truck" value={vehicle} onChange={(e) => setVehicle(e.target.value)} />
            </div>
            <div className="dom-form-field">
              <label className="dom-form-label">Registration Plate</label>
              <input type="text" placeholder="e.g. LX68 XYZ" value={plate} onChange={(e) => setPlate(e.target.value)} />
            </div>
            {!record.isSupervisor && (
              <div className="dom-form-field">
                <label className="dom-form-label">Route Sort</label>
                <select value={routeSort} onChange={(e) => setRouteSort(e.target.value as 'yes' | 'no')}>
          ...
          <div className="dom-form-actions wp-modal-actions">
            <button type="button" className="styled-button styled-button--danger" onClick={onUnassign}><i className="bi bi-trash" /> Unassign</button>
            <div className="wp-modal-actions-right">
              ...
              <button type="submit" className="styled-button styled-button--primary">Save</button>
            </div>
          </div>
        </form>
      </div>
