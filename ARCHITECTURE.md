...
2. **Live Service** at Depot MSE. `live-service.js` renders the `Pre-12`, `ASR`, and `DSR` groups as a dispatch carousel: one route at a time with its current delivery in focus, assigned MSE driver and vehicle, full delivery queue and progress. It advances the simulated operation and carousel every 7 seconds unless `prefers-reduced-motion` is enabled.
